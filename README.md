...
## Intenção Futura: Dockerização do Projeto

Uma das melhorias planejadas para o futuro é a **dockerização** do projeto. A intenção é encapsular toda a aplicação em containers Docker, o que tornará a configuração e execução do ambiente de desenvolvimento e produção mais simples e consistente. Com o uso de Docker, será possível garantir que todos os componentes da aplicação, incluindo as dependências e o banco de dados PostgreSQL, rodem de forma padronizada, independentemente do sistema operacional ou das configurações da máquina do desenvolvedor.

Essa abordagem também facilitará o processo de deployment, permitindo que a aplicação seja facilmente movida entre diferentes ambientes, desde o desenvolvimento até a produção, com mínimo esforço de configuração. A dockerização deve incluir arquivos `Dockerfile` para a aplicação e um `docker-compose.yml` para orquestrar os containers, incluindo o banco de dados, criando assim uma infraestrutura que seja robusta, escalável e simples de replicar.
